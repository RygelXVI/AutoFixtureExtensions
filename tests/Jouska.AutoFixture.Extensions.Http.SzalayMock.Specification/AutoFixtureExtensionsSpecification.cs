...
namespace Jouska.AutoFixture.Extensions.Http.SzalayMock.Specification;
...
public class AutoFixtureExtensionsSpecification
{
    ...
    {
        var path = "http://localhost/api/users/*";
        var expected = "blah blah blah";
        var httpMock = new MockHttpMessageHandler();
     
        httpMock
            .When(path)
            .Respond("application/text", expected);

        var fixture = new Fixture().WithHttpClientFactory(NamedHttpClientFactoryTestSubject.ClientName, httpMock);

        var testSubject = fixture.Freeze<NamedHttpClientFactoryTestSubject>();

        Assert.NotNull(testSubject);
        Assert.NotNull(testSubject.HttpClient);
    ...
    public async Task Can_register_single_http_mock_as_named_client()
    {
        ...
        httpMock
            .When(path)
            .Respond(System.Net.HttpStatusCode.UseProxy);
        ...
        var actual = await httpClient.GetAsync(path, TestContext.Current.CancellationToken);
    ...
    public async Task Can_register_multiple_httpmocks_as_named_clients()
    {
        ...
        var httpMock2 = new MockHttpMessageHandler();
        httpMock2
            .When(path)
            .Respond(System.Net.HttpStatusCode.PreconditionFailed);
        ...
        var actual = await client1.GetAsync(path, TestContext.Current.CancellationToken);
        Assert.Equal(System.Net.HttpStatusCode.UseProxy, actual.StatusCode);

        var client2 = factory.CreateClient(clientName2);
        var actual2 = await client2.GetAsync(path, TestContext.Current.CancellationToken);
    ...
    public async Task Can_register_httpmock_as_client()
    {
        ...
        var actual = await testSubject2.HttpClient.GetAsync(path, TestContext.Current.CancellationToken);
    ...
    public async Task Can_register_dictionary_of_httpmocks()
    {
        ...
        var clients = new Dictionary<string, MockHttpMessageHandler>
        {
